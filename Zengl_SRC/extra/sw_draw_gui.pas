...
var
  fontUse: zglPFont;
  JoyArrow: zglPTexture;          // текстуры для стрелок
  ...
  // цветовая схема меню окружности
...
uses
  zgl_text, sw_touch_menu, zgl_math_2d, zgl_sprite_2d, zgl_keyboard, zgl_fx,
  {$IFNDEF USE_GLES}
  zgl_opengl_all
  {$ELSE}
  zgl_opengles_all
...
var
  ...
  r: zglTRect;
begin
  setTextScale(TouchKey.textScale / 10);
  setTextColor(MenuColorText);
...
var
  ...
  r: zglTRect;
begin
  setTextScale(TouchKeySymb.textScale / 10);
  setTextColor(MenuColorText);
  ...
  Begin
    ...
    if (i = _home) or (i = _end) then
      setTextScale(TouchKeySymb.textScale / 20);
