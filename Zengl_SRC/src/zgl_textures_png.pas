...
begin
  ...
  repeat
    ...
    //                  hIST - хранение гистограммы или общего содержани€ каждого цвета в изображении
    // и ещЄ тонна всего...
    {
      ¬спомогательные чанки
        bKGD Ч этот чанк задает основной фоновый цвет.
        cHRM используетс€ дл€ задани€ CIE 1931 цветового пространства.
        gAMA Ч определ€ет гамму.
        hIST Ч в этом чанке может хранитьс€ гистограмма или общее содержание каждого цвета в изображении.
        iCCP Ч цветовой профиль ICC
        iTXt Ч содержит текст в UTF-8, возможно сжатый, с необ€зательной €зыковой меткой. iTXt чанк с
              ключевым словом 'XML:com.adobe.xmp' может содержать Extensible Metadata Platform (XMP).
        pHYs Ч содержит предполагаемый размер пиксел€ и/или отношение сторон изображени€.
        sBIT (significant bits) Ч определ€ет Ђцветовую точностьї (color-accuracy) изображени€ (черно-белое,
              полный цвет, черно-белое с прозрачностью и т.д.), дл€ более простого декодировани€.
        sPLT Ч предлагает палитру дл€ использовани€, если полный спектр цветов недоступен.
        sRGB Ч свидетельствует о использовании стандартной sRGB схемы.
        sTER Ч индикатор стереоскопических изображений.
        tEXt Ч может содержать текст в ISO/IEC 8859-1 формате, с одной name=value парой дл€ каждого чанка.
        tIME Ч хранит дату последнего изменени€ изображени€.
        tRNS Ч содержит информацию о прозрачности.
        zTXt Ч сжатый текст, с теми же ограничени€м, что и tEXt.
    }
